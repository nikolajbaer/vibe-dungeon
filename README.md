...
- **Placement/phone-first**: HUD elements are positioned `fixed`, styled in `index.html`'s inline `<style>` block (matching the existing convention for the controls hint / touch joysticks), and account for `env(safe-area-inset-bottom)` since this is a phone-first game. The health bar sits bottom-center; the two touch-joystick pads own the bottom corners, so keep new bottom-row elements narrow enough to fit between them.
