...
**Implementation note (deviation):** the `{w, d, h}` footprint is implemented literally as width (local x) × depth (local z) × height, per the bulleted definition above — the "3 wide, 2 tall, 3 long" aside on the `great_hall` example reads as a wording slip (it labels the depth slot "tall" and the height slot "long"), not a different field order. Both shipped tile types (`src/level/tiles.ts`) end up with the same `h=3` (9m) ceiling as a result; if a future task wants shorter corridors than rooms, that's a one-line change to `HALLWAY.h`, not a format change.
