...
key = np.array([-.5, .75, 1.]); key /= np.linalg.norm(key)
...
def render(vertices, width, height, az, el, scale, center, floor, target_y=0):
    ...
    for f, face in enumerate(faces):
        ...
        shade = .44 + .67*max(0,normal@key) + .17*max(0,normal@fill)
        if textured[f]:
...
sheet.paste(render(data['still'],600,980,0,.055,460,300,900),(0,145))
sheet.paste(render(data['still'],650,980,.62,.11,460,340,900),(570,145))
sheet.paste(render(data['still'],590,660,.26,.035,1950,295,600,1.49),(1210,215))
...
label(draw,'Slanted brow geometry', (1270,965),small_font)
label(draw,'Cutout rib texture', (1270,1000),small_font)
