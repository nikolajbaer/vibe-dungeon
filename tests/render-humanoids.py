...
species = ['attack', 'attack', 'attack'] if attack else ['idle', 'hit', 'death'] if actions else ['human', 'elf', 'goblin', 'dwarf']
...
images = []
for f in range(100 if attack else 90 if actions else 32):
    im = Image.new('RGB',(1200,740),(23,29,39));draw=ImageDraw.Draw(im)
    draw.text((42,32),'HUMANOID / BASE 01',font=big,fill=(230,237,248))
    draw.text((43,79),'One-handed short-sword stab / crouched guard / compact recovery' if attack else 'Loiter / Take damage / Death — actual rig animation' if actions else '1,580 triangles  /  23 bones  /  shared skeleton  /  walk cycle',font=small,fill=(152,173,200))
    for col,(s,d) in enumerate(zip(species,data)):
        ...
        x0=(240+col*400) if attack else (200+col*400) if actions else (155+col*297);ground=625;scale=225 if attack else 250
        ...
        label = ['FRONT','THREE-QUARTER','SIDE'][col] if attack else {'idle':'LOITER','hit':'TAKE DAMAGE','death':'DEATH'}.get(s,s.upper())
    ...
    images.append(im)
images[32 if attack else 45 if actions else 0].save(root/('humanoid-attack.png' if attack else 'humanoid-actions.png' if actions else 'humanoid-lineup.png'))
output = root/('humanoid-attack.gif' if attack else 'humanoid-actions.gif' if actions else 'humanoid-walk.gif')
