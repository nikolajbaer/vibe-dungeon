...
species = ['idle', 'hit', 'death'] if actions else ['human', 'elf', 'goblin', 'dwarf']
...
images = []
for f in range(90 if actions else 32):
    im = Image.new('RGB',(1200,740),(23,29,39));draw=ImageDraw.Draw(im)
    draw.text((42,32),'HUMANOID / BASE 01',font=big,fill=(230,237,248))
    draw.text((43,79),'Loiter / Take damage / Death — actual rig animation' if actions else '1,580 triangles  /  23 bones  /  shared skeleton  /  walk cycle',font=small,fill=(152,173,200))
    for col,(s,d) in enumerate(zip(species,data)):
        ...
        x0=(200+col*400) if actions else (155+col*297);ground=625;scale=250
        ...
        label = {'idle':'LOITER','hit':'TAKE DAMAGE','death':'DEATH'}.get(s,s.upper())
    ...
    images.append(im)
images[45 if actions else 0].save(root/('humanoid-actions.png' if actions else 'humanoid-lineup.png'))
output = root/('humanoid-actions.gif' if actions else 'humanoid-walk.gif')
