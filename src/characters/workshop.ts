...
import { createSkeletonWarriorRig } from './skeletonWarrior';
type WorkshopSpecies = HumanoidSpecies | 'skeleton';
const params = new URLSearchParams(location.search);
const makeRig = (species: WorkshopSpecies, weapon: HumanoidOptions['weapon']) => species === 'skeleton' ? createSkeletonWarriorRig({weapon}) : createHumanoidBase({species,weapon});
...
let rig=makeRig(params.get('species') === 'skeleton' ? 'skeleton' : 'human','shortSword'),mixer=new THREE.AnimationMixer(rig.mesh),helper=new THREE.SkeletonHelper(rig.mesh),paused=false,showBones=false;
scene.add(rig.mesh,helper);helper.visible=false;
const select=document.querySelector<HTMLSelectElement>('#species')!;
select.add(new Option('skeleton warrior','skeleton'));
if(params.get('species') === 'skeleton') select.value='skeleton';
...
if(params.get('clip') && Array.from(clip.options).some(option=>option.value===params.get('clip'))) clip.value=params.get('clip')!;
...
async function exportCharacter(species:WorkshopSpecies){const asset=makeRig(species,'shortSword');const data=await new GLTFExporter().parseAsync(asset.mesh,{binary:true,animations:Object.values(asset.clips)});return Array.from(new Uint8Array(data as ArrayBuffer));}
document.querySelector<HTMLButtonElement>('#export')!.onclick=async()=>{const bytes=await exportCharacter(select.value as WorkshopSpecies);const url=URL.createObjectURL(new Blob([new Uint8Array(bytes)],{type:'model/gltf-binary'}));const a=document.createElement('a');a.href=url;a.download=`humanoid-${select.value}.glb`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);};
